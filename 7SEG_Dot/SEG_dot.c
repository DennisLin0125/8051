/*-----------------------------------------------
  名稱：多位數碼管動態掃瞄顯示變化數據 顯示小數點方法
  編寫：dennis.lin
  日期：2020.9.5
  修改：無
  內容：多位數碼管分別顯示不同數字，這種掃瞄顯示方式成為動態掃瞄，並不停變化賦值
        共陰數碼管只要在對應的數字後面 | 0x80 即可顯示小數點 共陽數碼管 則 &0x7F 
...
main()
{
	...
	{
		j++;
		if(j==100)  //檢測目前數值顯示了一小段時間后，需要顯示的數值加1，實現數據顯示的變化
		{
			j=0;
			num++;
			if(num==1000) num=0; //用於顯示0~999
			
		}

		TempData[0]=dofly_DuanMa[num/100%10];      //取百位數  
		TempData[1]=dofly_DuanMa[num/10%10]|0x80;  //取十位數 共陰數碼管只要在對應的數字後面 | 0x80 即可顯示小數點
		TempData[2]=dofly_DuanMa[num%10];          //取個位數
		Display(5,3);
	}
}
/*------------------------------------------------
