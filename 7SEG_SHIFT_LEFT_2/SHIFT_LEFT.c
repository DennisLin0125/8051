/*-----------------------------------------------
  ...
  日期：2020.9.7
  修改：無
  內容：多位數碼管分別顯示不同數字，這種掃瞄顯示方式成為動態掃瞄，並連續向左顯示
...
main()
{
	...
	unsigned char *pa=TempData,*pb=dofly_DuanMa;//定義2個指針，並賦值2個數組初始地址
	  //*pa=TempData 等效于 *pa=&TempData[0]
	while(1)
	{

		j++;
		if(j==10)  //用於延時，改變值可以改變延時長度
		{
			j=0;    //延時到重新賦值
			for(i=0;i<(8-k);i++) //循環把顯示的字元值賦值 給臨時緩衝陣列TempData						
			{
				*(pa+k+i)=*(pb+i+m);//使用指針操作賦值
			}
			
			for(i=0;i<m;i++)//8位左移出去，右邊空出來的數/碼管需要熄滅，
			{ 
				*(pa+8-m+i)=0;  //所有賦值0 m表示移出位置，同時也表示右邊需要熄滅位數
			}			
							  		  
			    
			if(k>0) k--;        //判斷這些值以便循環顯示
