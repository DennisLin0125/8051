/*-----------------------------------------------
  ...
  日期：2020.10.8
  修改：無
  內容：DS1302實時時鐘數碼管顯示，只顯示時間。並通過4個按鍵加減小時、分鐘，其他參數調節自行新增
...
void main (void)
{
	...
	while (1)         //主循環
	{
		...
		switch(num)
		{
			...
			case 2:time_buf1[4]--;
				if(time_buf1[4]==255) time_buf1[4]=23; 	 
				Ds1302_Write_Time();
				break; //正常時間 小時減1
			...
			case 4:time_buf1[5]--;
				if(time_buf1[5]==255) time_buf1[5]=59;	 
				Ds1302_Write_Time();
				break; //分減1
			default:break;
		}
		if(ReadTimeFlag==1)
		{
			ReadTimeFlag=0;
			Ds1302_Read_Time();
			//數據的轉換，因我們採用數碼管0~9的顯示,將數據分開
			TempData[0]=dofly_DuanMa[time_buf1[4]/10];	//時		
